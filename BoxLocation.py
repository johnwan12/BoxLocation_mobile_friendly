# BoxLocation.py — Professional UI + Simple Workflow + Sticky Header + Tabs (NO DEBUG)
# ============================================================
...
# ✅ QR is generated by QuickChart (no qrcode dependency)
#    - fetch_bytes() disables proxies to reduce SSL issues
...
def fetch_bytes(url: str, timeout: int = 12) -> bytes:
    """
    Fetch bytes safely. Disables proxies to reduce SSL WRONG_VERSION_NUMBER issues
    commonly caused by intercepting proxies.
    """
    req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0"})
    opener = urllib.request.build_opener(urllib.request.ProxyHandler({}))  # disable proxies
...
def ensure_boxnumber_header(service):
    # boxNumber must have these columns for this workflow
...
def insert_boxnumber_row(service, study_id_value: str, box_number_value: str):
    """
    After saving Freezer record:
      - INSERT a NEW row into boxNumber:
        StudyID   = Prefix + Tube suffix
        BoxNumber = TubeAmount
    """
...
# ============================================================
# Sticky Header: title + status + context
# ============================================================
